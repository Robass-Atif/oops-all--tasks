...
namespace Pacman_with_Oop
{
    ...
    {
        private  GameCell[,] grid = new GameCell[24, 70];
        private int row;
        private int col;
        ...
        public  void loadGrid(string fileName)
        {
            if (File.Exists(fileName))
            {
                ...
                while ((data = file.ReadLine()) != null)
                {
                    ...
                    {
                        GameCell cell = new GameCell(row, col, getGameObject(data[col]));
        ...
        public  void printMaze()
        {
            for (int i = 0; i < grid.GetLength(0); i++)
            {
                for (int j = 0; j < grid.GetLength(1); j++)
                {
                    ...
                    Console.Write(Grid[i, j].CurrentGameObject.DisplayCharacter);
                }
                Console.WriteLine();
            }
